...
struct is_value : std::integral_constant<bool, std::is_integral<ValueT>::value or std::is_floating_point<ValueT>::value>
{};
 ...
 */
template <typename ValueT> struct Invalid
{
 ...
 *        to a goal state.
 */
template <typename ValueT, typename HeuristicT = ValueT> class HeuristicValue
{
public:
  ...
  constexpr ValueT h() const { return this->heuristic; }

  constexpr bool operator<(const HeuristicValue& other) const { return this->value < other.value; }
  ...
  constexpr HeuristicValue operator-(int) const { return HeuristicValue{-this->value, -this->heuristic}; }
  ...
  template <typename ScalarT> constexpr const HeuristicValue& operator*=(const ScalarT& scaling)
  {
    this->value *= scaling;
  ...
  friend constexpr HeuristicValue operator*(const HeuristicValue& lhs, const ScalarT& scaling)
  {
    return HeuristicValue{
      construct_raw, static_cast<ValueT>(lhs.value * scaling), static_cast<HeuristicT>(lhs.heuristic * scaling)};
  }
  ...
  {
    return HeuristicValue{
      construct_raw, static_cast<ValueT>(rhs.value * scaling), static_cast<HeuristicT>(rhs.heuristic * scaling)};
  }
...
private:
   ...
   */
  constexpr HeuristicValue(construct_raw_t, ValueT _value, HeuristicT _heuristic) : value{_value}, heuristic{_heuristic}
  {}
...
template <typename ValueT, typename HeuristicT> struct Null<HeuristicValue<ValueT, HeuristicT>>
{
  static constexpr HeuristicValue<ValueT, HeuristicT> value{Null<ValueT>::value, Null<HeuristicT>::value};
};


template <typename ValueT, typename HeuristicT> struct Invalid<HeuristicValue<ValueT, HeuristicT>>
{
